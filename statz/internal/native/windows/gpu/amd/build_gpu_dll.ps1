# Build script for AMD GPU monitoring DLL
# Requires GCC (MinGW-w64) to be installed and in PATH

Write-Host "Building AMD GPU monitoring DLL..." -ForegroundColor Green
...
# Compile the DLL
Write-Host "Compiling gpu_usage.c to amd_gpu_monitor.dll..." -ForegroundColor Yellow
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "Success! DLL created at: dist/amd_gpu_monitor.dll" -ForegroundColor Green
        Write-Host ""
        ...
        Write-Host "  dll = ctypes.CDLL('./dist/amd_gpu_monitor.dll')" -ForegroundColor Gray
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "Note: For full functionality, install AMD AGS library" -ForegroundColor Yellow
        Write-Host "Otherwise, basic functionality via Performance Counters" -ForegroundColor Yellow
        
        # Show file size
        $fileInfo = Get-Item "dist/amd_gpu_monitor.dll"
...
try {
    & gcc $testCompileArgs
    if ($LASTEXITCODE -eq 0) {
        Write-Host "Success! Test executable created at: dist/amd_gpu_test.exe" -ForegroundColor Green
        Write-Host "Run it to test AMD GPU monitoring functionality." -ForegroundColor Cyan
        
        $testFileInfo = Get-Item "dist/amd_gpu_test.exe"
